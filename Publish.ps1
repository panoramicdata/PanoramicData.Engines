<#
.SYNOPSIS
    Publishes the PanoramicData.Engines package to NuGet.org
...
.DESCRIPTION
    ...
    4. Runs unit tests when a test project exists (unless -SkipTests is specified)
    5. Builds and publishes to nuget.org
...
$projectName = 'PanoramicData.Engines'
$projectPath = Join-Path $PSScriptRoot "$projectName\$projectName.csproj"
$testProjectPath = Join-Path $PSScriptRoot "$projectName.Test\$projectName.Test.csproj"
...
if (-not $SkipTests) {
    if (Test-Path $testProjectPath) {
        ...
        dotnet test $testProjectPath --configuration Release --no-restore
        if ($LASTEXITCODE -ne 0) {
        ...
        Write-Warning 'No test project was found. Skipping unit tests.'
    }
}
else {
...
$packagePath = Join-Path $PSScriptRoot "$projectName\bin\Release\$projectName.$version.nupkg"
...
Write-Information "Successfully published $projectName version $version to nuget.org!" -InformationAction Continue
exit 0
