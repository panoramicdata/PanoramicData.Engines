...
# PSScriptAnalyzer flags Write-Host (PSAvoidUsingWriteHost) because it cannot be redirected or
# captured. Write-Information with -InformationAction Continue reaches the console the same way,
# and ANSI escapes preserve the colouring these messages relied on.
$script:AnsiColours = @{
	Red    = "$([char]27)[31m"
	Yellow = "$([char]27)[33m"
	Cyan   = "$([char]27)[36m"
...
function Write-Message {
	...
	$text = if ($Colour) { "$($script:AnsiColours[$Colour])$Message$([char]27)[0m" } else { $Message }
